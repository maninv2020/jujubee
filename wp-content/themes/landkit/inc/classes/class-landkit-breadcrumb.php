...
class Landkit_Breadcrumb {
	...
	public function get_breadcrumb() {
		return apply_filters( 'landkit_get_breadcrumb', $this->crumbs, $this );
	}
	...
	protected function add_crumbs_404() {
		$this->add_crumb( __( 'Error 404', 'landkit' ) );
	}
	...
	protected function add_crumbs_single( $post_id = 0, $permalink = '' ) {
		...
		if ( 'product' === get_post_type( $post ) ) {
			...
			$terms = wc_get_product_terms(
				...
				'product_cat',
				apply_filters(
					'landkit_breadcrumb_product_terms_args',
					array(
						'orderby' => 'parent',
						'order'   => 'DESC',
					)
			...
			if ( $terms ) {
				$main_term = apply_filters( 'landkit_breadcrumb_main_term', $terms[0], $terms );
				$this->term_ancestors( $main_term->term_id, 'product_cat' );
				$this->add_crumb( $main_term->name, get_term_link( $main_term ) );
			}
		} elseif ( 'docs' === get_post_type( $post ) ) {
	...
	protected function add_crumbs_product_tag() {
		...
		$this->add_crumb( sprintf( __( 'Products tagged &ldquo;%s&rdquo;', 'landkit' ), $current_term->name ), get_term_link( $current_term, 'product_tag' ) );
	}
	...
	protected function add_crumbs_tag() {
		...
		$this->add_crumb( sprintf( __( 'Posts tagged &ldquo;%s&rdquo;', 'landkit' ), single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	protected function add_crumbs_author() {
		...
		$this->add_crumb( sprintf( __( 'Author: %s', 'landkit' ), $userdata->display_name ) );
	}
	...
	protected function endpoint_trail() {
		$endpoint       = ( landkit_is_woocommerce_activated() && is_wc_endpoint_url() ) ? WC()->query->get_current_endpoint() : '';
	...
	protected function search_trail() {
		if ( is_search() ) {
			...
			$this->add_crumb( sprintf( __( 'Search results for &ldquo;%s&rdquo;', 'landkit' ), get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
	...
	protected function paged_trail() {
		if ( get_query_var( 'paged' ) ) {
			...
			$this->add_crumb( sprintf( __( 'Page %d', 'landkit' ), get_query_var( 'paged' ) ) );
		}
	}
}
